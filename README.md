...
当前仓库已经完成一版可运行的本地 MVP。项目采用 Taro + React + TypeScript，可构建为微信小程序和 H5；默认使用本地 Mock 完成识别与诊断流程，个人数据保存在当前设备。仓库同时提供可在本地启动的厂商无关 Express Mock 接口，但尚无已部署的数据后端，真实 AI 也尚未完成选型和接入。

## 当前功能

| 模块 | 已实现能力 |
| --- | --- |
| 首页 | 本地植物搜索、识别/诊断/花园入口、今日与逾期任务、每日提示、静态常见植物推荐 |
| 植物识别 | 拍照或选择图片、Mock 或后端模式、结果来源与可信度、养护建议、加入花园、识别历史 |
| 植物诊断 | 图片和文字可单独或组合提交、Mock 或后端模式、结果来源、处理步骤、关联花园植物、诊断历史 |
...
- 真实 AI 接入数量为 0。团队提出的百度植物识别和百度云上 DeepSeek 目前只是待核验候选，需要先确认准确产品、API 文档、鉴权方式和模型 ID；当前代码不绑定任何模型厂商。
- 尚无已部署的 CloudBase 或其他数据后端，不支持登录、云同步和多设备共享；仓库中的 `server/` 只是本地 Express Mock 接口和未来服务端适配骨架。
- 已能创建并在应用内查看养护任务，但尚未发送微信订阅消息。
- 当前内置知识库只包含少量演示植物，不是完整植物百科。
- 正式小程序 AppID 已申请并仅在本地配置；微信认证、名称、备案备注和隐私配置仍在推进，真机全流程与发布验收尚未完成。
...
1. 使用已申请的正式 AppID 完成开发者权限、认证/名称/备案/隐私配置以及真机流程测试。
2. 确定后端方案，完成任务云端存储、定时调度和微信订阅消息。
3. 核验百度植物识别和百度云上 DeepSeek 的准确服务信息，再按准确率、图文能力、隐私、稳定性和成本完成 AI 选型及服务端适配。
4. 完成热门植物详情跳转、低可信度候选、识别结果确认后入历史、诊断转提醒和湿度建议等产品缺口。
5. 完善权限拒绝、网络/存储失败和数据迁移等边界场景，扩充有可靠来源和授权记录的植物知识数据。
