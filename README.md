...
当前仓库已经完成一版可运行的本地 MVP。项目采用 Taro + React + TypeScript，可构建为微信小程序和 H5；默认使用本地 Mock 完成识别与诊断流程，个人数据保存在当前设备。仓库同时保留厂商无关的 Express 接口骨架，但真实 AI 尚未选型和接入。
...
| 植物识别 | 拍照或选择图片、Mock/AI 服务切换、结果来源与可信度、养护建议、加入花园、识别历史 |
| 植物诊断 | 图片和文字可单独或组合提交、Mock/AI 服务切换、结果来源、处理步骤、关联花园植物、诊断历史 |
...
- 默认配置下，植物识别和问题诊断是本地模拟结果，不代表真实模型能力，也不能替代专业检测。
- 真实 AI 服务尚未选型；当前代码不绑定任何模型厂商，识别和诊断均明确标记为模拟结果。
...
| 自动检查 | GitHub Actions |
| 可选后端 | Node.js 22、Express，暂未绑定 AI 厂商 |
...
默认 Mock 模式不需要 CloudBase、数据库、AI 密钥或单独启动后端服务。需要联调真实 AI 时才安装并启动 `server/`。
...
复制根目录 `.env.example` 为不提交的 `.env`。默认配置为：
...
这时图片不会上传。联调后端时，将 `TARO_APP_API_BASE_URL` 配置为 API 地址并把 `TARO_APP_USE_MOCK` 改为 `false`。微信真机必须使用已部署的 HTTPS 地址，不能使用 `localhost`，还要在微信公众平台配置 request 合法域名。

后端配置位于 `server/.env`，可参考 `server/.env.example`。当前后端只返回经过校验的 Mock 数据；模型选型后，API 密钥只能放在服务端环境变量中：

```bash
npm run dev --prefix server
```

生产部署必须配置 `CORS_ORIGINS`、合理的限流参数和可信反向代理设置。当前限流用于降低滥用风险；正式用户体系建立后仍需增加基于微信身份的服务端鉴权。
...
3. 按准确率、图文能力、隐私、稳定性和成本完成 AI 服务选型，再实现服务端适配器。
