...
| 模块 | 已实现能力 |
| --- | --- |
| 首页 | 火山方舟 AI 植物文字搜索（本地结果自动回退）、识别/诊断/花园入口、今日与逾期任务、每日提示、可进入知识详情的常见植物推荐 |
...
- 首页文字搜索、植物识别和问题诊断已实现火山方舟 Doubao-Seed-2.0-Mini 适配器、超时、响应校验和 Mock 回退；本地真实密钥联调已完成，HTTPS 部署及微信真机验证仍需完成。AI 结果仅供参考，不能替代专业检测。
...
- 首页搜索会调用火山方舟生成植物资料并标记结果来源；AI 结果和下方常见植物卡片均可进入知识详情，AI 不可用时回退到本地知识库。
...
2. 部署后端到 HTTPS 环境，配置微信合法域名，让真机也能使用 AI 文字搜索、识别与诊断。
3. 为公网 AI 接口补充微信身份鉴权、按用户限流和费用告警。
